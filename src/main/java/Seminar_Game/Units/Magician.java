...
public class Magician extends Base_Unit{
    ...
    public Magician(double x, double y, String type, String name, int health, int damageMin, int damageMax,int speed, int def, int attack, int mana, int spell, int hp, int damage) {
        super(x, y, type, name, health, damageMin, damageMax, speed, def, attack);
        this.mana = mana;
        this.damage = damage;
    }
    public Magician(double x, double y, String  name) {
    ...
    public void step(ArrayList enemyTeam, ArrayList ownTeam) {
        ...
        if (this.health > 0 &&  getClosestWeakest(ownTeam).health <= 0){
            ...
            if (coords.getDistance(target.getX(), target.getY()) <= 3) {
                System.out.println(this.type+" "+ name+" I`m striking "+target.type +" "+ target.name+". His health "+target.health+" now");
                target.setHealth(coords.getClosest(enemyTeam).getHealth() - this.damage);
            }
        }
    }
    @Override
    public String getInfo() {return type+" " +name+"- health: "+health+", mana: "+mana+"        "+", "+"X: "+this.getX()+", Y: "+this.getY();}
